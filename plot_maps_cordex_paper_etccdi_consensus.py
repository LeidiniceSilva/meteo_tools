...
def read_consensus(folder, varname):
    ds = xr.open_dataset(Path(base) / folder / "consensus.nc")
    return ds["lon"], ds["lat"], ds[varname]
...
# load consensus
_, _, cdd_cam_cons = read_consensus("CDD_CAM", "ds_anom_consensus")
_, _, cdd_sam_cons = read_consensus("CDD_SAM", "ds_anom_consensus")

_, _, txx_cam_cons = read_consensus("TXx_CAM", "TXx_anom_consensus")
_, _, txx_sam_cons = read_consensus("TXx_SAM", "TXx_anom_consensus")
...
# save
plt.savefig("TXx_CDD_CAM-SAM_CORDEX_domains_consensus.png", dpi=400, bbox_inches="tight")
plt.show()
